...
| 043 | 忆阻混沌电路的设计及其在图像加密算法中的应用 | 叶晓林 (Ye Xiaolin) | 2022 | 大连海事大学博士论文 | 多忆阻器文氏桥电路+忆感器超多稳定系统+多涡卷动态加密 | [paper_043_忆阻混沌电路设计及图像加密应用.md](paper_043_忆阻混沌电路设计及图像加密应用.md) |
| 044 | 非对称细胞神经网络稳定平衡点的存在性 | 张发明 | 2005 | 电子测量与仪器学报 19(3) | 非对称CNNs稳定平衡点存在性（$\mathcal{R}$/$\mathcal{F}$类矩阵条件推广M-矩阵条件） | [paper_044_非对称CNNs稳定平衡点存在性.md](paper_044_非对称CNNs稳定平衡点存在性.md) |
| 045 | A fractional-order ship power system with extreme multistability | Zhang H, Sun K, He S | 2021 | Nonlinear Dynamics | 分数阶船舶电力系统极端多稳态（ADM三角函数优化+角度不稳定性+自然极端多稳态+DSP实现） | [paper_045_分数阶船舶电力系统极端多稳态.md](paper_045_分数阶船舶电力系统极端多稳态.md) |
| 046 | 基于混沌符号动力学的图像加密研究 | 王金铭, 虞飞华 | 2011 | 计算机工程 37(2) | 混沌符号动力学单密钥→多密钥图像加密（倒锯齿映射+符号运算+位置/像素值双重置乱） | [paper_046_混沌符号动力学图像加密.md](paper_046_混沌符号动力学图像加密.md) |
...
- **032号论文**提出了基于符号函数组合的简洁控制函数，实现了3D和4D Jerk系统的多涡卷扩展；核心创新包括单涡卷和双涡卷两种配置的统—框架、高维系统扩展可行性验证、TMS320F28335 DSP硬件实现；涡卷数量可按(N+2)规律灵活调节，最多实现4×3×2×3=72个涡卷；为多涡卷混沌系统的工程应用提供了新的简洁方法
...
| **多稳态类型** | 极端多稳态（无穷多） | 有限多稳态（共存吸引子） | **异质+同质多稳态共存** | **初始增强极端多稳态** | **同质极端多稳态（无穷多）** |
| **增强机制** | 无 | 无 | 无 | **初始条件控制偏移（周期=2）** | **迭代次数变量引入** |
| **平衡点** | 平面固定点集（无穷多） | 无固定点（隐藏吸引子） | **无穷多个固定点** | **平面固定点集（无穷多）** | **无固定点（隐藏吸引子）** |
| **超混沌** | 是（2个正LE） | 是（多正LE） | **是（2个正LE）** | **是（2个正LE）** | **是（2个正LE）** |
| **图像加密** | 未涉及 | DNA编码加密 | **Cube+Arnold+GF257三层加密** | **未涉及（PRNG应用）** | **MGM-IEA算法+对角扩散** |
| **硬件实现** | STM32微控制器 | TMS320F28335 DSP | **TMS320F28335 DSP** | **STM32F407微控制器** | **STM32F103+TLV5618** |
| **期刊级别** | CSF (二区) | IEEE TIE (一区) | **CSF (二区)** | **CSF (二区)** | **CSF (二区)** |
...
*最后更新：2026-09-19
*本次更新：新增论文089精读笔记 - Dual neural network heterogeneous coupled complex multiscroll system with extreme multistability and its application in image encryption (Nonlinear Dynamics, 2026)，Yin等人针对现有忆阻Hopfield神经网络研究集中于单网络结构、子网络间信息交换机制缺乏探索的问题，提出了一种新型多段线性忆阻器模拟神经突触，并构建了双神经网络异质耦合复杂多涡卷系统(DNNHCMS)。核心贡献：(1) 通过突触耦合连接两个非对称子神经网络，发现耦合强度是决定系统动力学特性的关键参数；(2) 在特定参数范围内可产生多个双涡卷吸引子，涡卷数量可通过忆阻器段数参数灵活调节，实现了"器件参数→吸引子形态"的直接可控映射；(3) 系统同时展现极端多稳态与灵活幅度控制，电路仿真与硬件实现双重验证；(4) 基于系统高复杂度混沌序列构建10个明文相关动态S盒，每轮置乱-扩散中动态选择S盒执行代换，兼顾安全性与效率。该论文与知识库中009号(Kong分数阶忆阻HNN+FPGA)、031/044号(CNN动力学与稳定性)、081号(ECC动态S盒)、003/004/007/016号(离散忆阻系统系列)形成互补——本文的独特价值在于突破单网络HNN构造范式，"双网络异质耦合"架构为"结构即密钥"提供了新的设计自由度。不足之处：摘要层面未见NIST随机性测试数据、双网络硬件开销需评估、动态S盒的密码学性质（非线性度/差分均匀度）未展开、与同类HNN加密工作的差异化优势需进一步论证。注：本笔记基于摘要与公开信息整理，具体数值指标建议精读原文后补充*
