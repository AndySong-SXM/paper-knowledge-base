...
*最后更新：2026-06-12
*本次更新：新增论文044精读笔记 - 非对称细胞神经网络稳定平衡点的存在性 (电子测量与仪器学报, 2005)，张发明利用饱和域特性和拟对角列支配矩阵与M-矩阵之间的关系，获得了非对称CNNs稳定平衡点存在的两个充分条件：(1) $A-I \in \mathcal{R}$（拟对角列支配矩阵类，推广了文献[5]的 $\mathcal{C}$ 类M-矩阵条件）；(2) $A-I \in \mathcal{F}$（与 $\mathcal{R}$ 类不可比较的独立矩阵类）。核心创新包括发现 $\mathcal{R}$ 类与 $\mathcal{F}$ 类的不可比较性、将文献[4,5,6]的结论统一为本文结果的特例、对二细胞CNNs完全稳定性给出严格证明。该论文与知识库中031号(王宏霞2003 CNN动力学行为)形成互补——031关注混沌产生条件，本文关注稳定性保证条件，共同构成了CNNs从稳定到混沌的完整动力学图景*
