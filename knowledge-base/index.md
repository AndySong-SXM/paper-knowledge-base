...
| 对比维度 | 003 - Bao et al. (2022) 并行双忆阻器 | 004 - Liu X et al. (2024) 离散忆阻器+忆感器 | **007 - Zhou et al. (2025) 异质同质多稳态** | **016 - Zhang et al. (2023) 初始增强极端多稳态** |
|---------|-------------------------------------|------------------------------------------|---------------------------------------------|--------------------------------------------------|
| **系统类型** | 并行双忆阻器超混沌映射 | 离散忆阻器+忆感器5-D超混沌映射 | **正弦-指数型离散忆阻器3-D超混沌映射** | **三角函数双忆阻器3-D超混沌映射** |
| **维度** | 2维 | 5维 | **3维** | **3维** |
| **忆阻器模型** | 双忆阻器并行（二次型） | 离散忆阻器+离散忆感器 | **指数-正弦组合型忆阻器** | **正弦+正弦/正弦+余弦型忆阻器** |
| **多稳态类型** | 极端多稳态（无穷多） | 有限多稳态（共存吸引子） | **异质+同质多稳态共存** | **初始增强极端多稳态** |
| **增强机制** | 无 | 无 | 无 | **初始条件控制偏移（周期=2）** |
| **平衡点** | 平面固定点集（无穷多） | 无固定点（隐藏吸引子） | **无穷多个固定点** | **平面固定点集（无穷多）** |
| **超混沌** | 是（2个正LE） | 是（多正LE） | **是（2个正LE）** | **是（2个正LE）** |
| **图像加密** | 未涉及 | DNA编码加密 | **Cube+Arnold+GF257三层加密** | **未涉及（PRNG应用）** |
| **硬件实现** | STM32微控制器 | TMS320F28335 DSP | **TMS320F28335 DSP** | **STM32F407微控制器** |
| **期刊级别** | CSF (二区) | IEEE TIE (一区) | **CSF (二区)** | **CSF (二区)** |
